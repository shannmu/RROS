...
void rust_helper_add_wait_queue(struct wait_queue_head *wq_head,
...
int rust_helper_wait_event_interruptible(struct wait_queue_head *wq_head,
					 bool condition)
{
...
int rust_helper_cpumask_next(int n, const struct cpumask *srcp)
{
	return cpumask_next(n, srcp);
}
EXPORT_SYMBOL_GPL(rust_helper_cpumask_next);
...
int rust_helper_cpumask_and(struct cpumask *dstp, const struct cpumask *src1p,
...
void *rust_helper_kthread_run(int (*threadfn)(void *data), void *data,
			      const char namefmt[], ...)
{
	va_list args;
...
void rust_helper__this_cpu_write(struct clock_proxy_device *pcp,
				 struct clock_proxy_device *val)
{
...
rust_helper__this_cpu_read(struct clock_proxy_device *pcp)
{
...
void rust_helper_init_irq_work(struct irq_work *work,
...
int rust_helper_proxy_set(ktime_t expires, struct clock_event_device *dev)
{
	struct clock_event_device *real_dev =
		container_of(dev, struct clock_proxy_device, proxy_device)
			->real_device;
	unsigned long flags;
	int ret;
...
rust_helper_dovetail_task_state(struct task_struct *task)
{
...
void rust_helper_raw_spin_unlock_irqrestore(hard_spinlock_t *lock,
					    unsigned long flags)
{
...
void rust_helper_list_splice_init(struct list_head *list,
...
__u16 rust_helper_vlan_dev_get_egress_qos_mask(struct net_device *dev,
					       u32 skprio)
{
...
void rust_helper_hash_add(struct hlist_head *hashtable, size_t length,
			  struct hlist_node *node, u32 key)
{
...
struct hlist_head *rust_helper_get_hlist_head(struct hlist_head *hashtable,
					      size_t length, u32 key)
{
...
unsigned long rust_helper_raw_copy_from_user(void *dst, const void *src,
					     unsigned long size)
{
...
unsigned long rust_helper_raw_copy_to_user(void *dst, const void *src,
					   unsigned long size)
{
...
void rust_helper_bitmap_copy(unsigned long *dst, const unsigned long *src,
			     unsigned int nbits)
{
...
void rust_helper_init_work(
	struct work_struct *work,
	void (*rust_helper_work_func)(struct work_struct *work))
{
...
void rust_helper__vlan_hwaccel_put_tag(struct sk_buff *skb, __be16 vlan_proto,
				       __u16 vlan_tci)
{
...
struct sk_buff *rust_helper_netdev_alloc_oob_skb(struct net_device *dev,
						 dma_addr_t *dma_addr)
{
...
struct net *rust_helper_dev_net(const struct net_device *dev)
{
	return dev_net(dev);
}
...
#include <linux/netdevice.h>
int rust_helper_dev_parse_header(const struct sk_buff *skb,
				 unsigned char *haddr)
{
...
bool rust_helper_dev_validate_header(const struct net_device *dev,
...
void rust_helper___vlan_hwaccel_put_tag(struct sk_buff *skb, __be16 vlan_proto,
					__u16 vlan_tci)
{
	__vlan_hwaccel_put_tag(skb, vlan_proto, vlan_tci);
}
