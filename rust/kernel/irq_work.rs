...
impl IrqWork {
    ...
    /// `irq_work_queue`: A method that queues the `IrqWork`. It calls the `bindings::irq_work_queue` function with a pointer to the `bindings::irq_work`. If the function returns `true`, it returns `Ok(0)`. Otherwise, it returns `Err(kernel::error::code::EINVAL)`.
