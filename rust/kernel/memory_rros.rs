...
use crate::{c_types, mm, prelude::*, premmpt, sync::SpinLock, vmalloc, sync::new_spinlock, error::code};
...
impl RrosHeap {
    ...
    pub fn init(&mut self, membase: *mut u8, size: SizeT) -> Result<usize> {
        ...
        let spinlock = new_spinlock!(1);
        let pinned = Box::pin_init(spinlock).unwrap();
        self.lock = Some(pinned);
    ...
    pub fn search_left_mergeable(&self, r: *mut RrosHeapRange) -> Option<*mut RrosHeapRange> {
        ...
        while !node.is_null() {
            let p = unsafe {crate::container_of!(node, RrosHeapRange, addr_node)};
            unsafe {
